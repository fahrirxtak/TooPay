...
const Navbar = () => {
  ...
  const SquirrelIcon = ({ className = "w-6 h-6" }) => (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor">
      ...
      <path
        d="M15 16c2-1 4-2 5.5-4.5 1.5-2.5 1-5 0-6.5-1 0.5-2 1.5-3 2.5-1 1-1.5 2-1.5 3 0 1.5 0.5 3 1 4.5 0.3 1 0.5 2 0.8 3-0.8-0.8-1.8-1.5-2.8-2z"
        opacity="0.8"
      />

      {/* Detail ekor tambahan untuk efek berbulu */}
      <path
        d="M16 14c1.5-0.5 3-1.5 4-3 0.8-1.2 0.8-2.5 0.3-3.5-0.5 0.3-1 0.8-1.5 1.3-0.8 0.8-1.3 1.5-1.3 2.2 0 1 0.3 2 0.5 3z"
        opacity="0.6"
      />
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center justify-between h-16 lg:h-20">
            ...
            {menuItems.map((item, index) => (
              ...
              >
                <button
                  ...
                  className={`flex items-center space-x-2 px-6 py-3 rounded-2xl text-sm font-medium transition-all duration-300 hover:scale-105 ${activeSection === item.id
                ...
                {item.hasDropdown && item.dropdownItems && (
                  <div
                    className={`absolute top-full left-1/2 transform -translate-x-1/2 mt-2 w-64 transition-all duration-300 ${activeDropdown === index
                    ...
                    <div className="bg-white/10 backdrop-blur-xl rounded-2xl border border-white/20 shadow-lg p-2 overflow-hidden">
                      {item.dropdownItems.map((dropdownItem, dropdownIndex) => (
                        <button
                          ...
                          className="relative flex items-center space-x-3 p-3 rounded-xl hover:bg-white/10 transition-all duration-200 group/item w-full text-left"
                        >
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-neutral-500 dark:text-neutral-400">{dropdownItem.description}</p>
                          </div>
                        </button>
                      ))}
                    </div>
          ...
          <div className="flex items-center space-x-4">
            ...
            <a
              ...
              className="hidden lg:flex items-center space-x-3 px-6 py-3 bg-gradient-to-r from-amber-500 via-orange-500 to-red-500 text-white rounded-2xl shadow-xl shadow-amber-500/40 hover:shadow-2xl hover:shadow-amber-500/60 hover:scale-105 transition-all duration-300 group relative overflow-hidden"
            >
      ...
      <div
        className={`lg:hidden absolute top-full left-0 w-full transition-all duration-500 ${isOpen ? "opacity-100 visible translate-y-0" : "opacity-0 invisible -translate-y-4"
          }`}
      >
          ...
          <div className="flex flex-col p-6 space-y-2">
            {menuItems.map((item, index) => (
              <button
                key={index}
                onClick={() => scrollToSection(item.id)}
                className={`flex items-center justify-between p-4 rounded-2xl font-medium transition-all duration-300 hover:scale-[1.02] text-left w-full ${activeSection === item.id
            ...
            <button
              ...
              className="flex items-center justify-center space-x-3 p-4 mt-4 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-2xl shadow-lg shadow-amber-500/30 hover:shadow-xl hover:shadow-amber-500/50 transition-all duration-300"
            >
